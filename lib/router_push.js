...
module.exports = {
    ...
    "onmessage": (message) => {
        ...
        if (!MessageTypes.hasOwnProperty(channel)) {
            consoleLogger.error("%s Invalid channel %s" + TAG + channel, 'websocketMessage');
            return;
        }
    ...
    "init": () => {
        ...
        consoleLogger.info("%s Init channels! %j" + TAG + receivers, 'websocketMessage');
    },
        ...
        if (receiver.hasOwnProperty("pushChannels")) {
            for (let x of receiver.pushChannels) {
                if (MessageTypes.hasOwnProperty(x)) {
                    ...
                    consoleLogger.error("%s Unable to register channel %s" + TAG + x, 'websocketMessage');
                }
            }
        }
    },
