...
module.exports = {
    ...
    "onmessage": (message) => {
        consoleLogger.debug('censusWebsocketMessage', JSON.stringify(message, null, 4));

        let channel = message.type;
        if (!MessageTypes.hasOwnProperty(channel)) {
            consoleLogger.error('censusWebsocketMessage', "%s Invalid channel %s", TAG, channel);
            return;
        }
    ...
    "init": () => {
        ...
        consoleLogger.info('pushChannels', "%s Init channels! %j", TAG, receivers);
    },
        ...
        if (receiver.hasOwnProperty("pushChannels")) {
            for (let x of receiver.pushChannels) {
                if (MessageTypes.hasOwnProperty(x)) {
                    ...
                    consoleLogger.error('pushChannels', "%s Unable to register channel %s", TAG, x);
                }
            }
        }
    },
