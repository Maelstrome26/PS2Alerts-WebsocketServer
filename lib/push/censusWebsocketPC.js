...
function initWebsocket(environment) {
    ...
    consoleLogger.status('censusEvents', 'Initializing websocket connection to Census (' + environment + ')');
    consoleLogger.status('censusEvents', path);
    ...
    websocketClient.on('open', function () {
        consoleLogger.status('censusEvents', 'Websocket Open!');
        fireMetagameSubscriptions();
    });

    websocketClient.on('error', function (error) {
        consoleLogger.error('censusEvents', 'WEBSOCKET ERROR: ' + error);

        websocketClient = null; // Destroy the websocket instance
        setTimeout(function() {
            consoleLogger.error('censusEvents', 'Reconnecting websocket...');
...
function sendMessage(message) {
    consoleLogger.debug('censusMessages', 'Sending to Census: %j', message);
...
function fireMetagameSubscriptions() {
    for (var key in config.supplementalConfig.pcWorlds) {
        ...
        consoleLogger.debug('censusEvents', 'Firing MetagameEvent Subscription - World: ' + world);
        sendMessage(message);
    }
}
