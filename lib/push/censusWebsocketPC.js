'use strict';
// Websocket to census

const routerPush = require('../router_push.js');
const config = require('../../config.js');
const consoleLogger = require('../debugger.js');

let ws = require('ws');
let websocketClient;

let subscriptions = {
    MetagameEvent: false, // We'll only be subscribing globally
    Death: {
        1: false,
        10: false,
        13: false,
        17: false,
        19: false,
        25: false
    },
    FacilityControl: {
        1: false,
        10: false,
        13: false,
        17: false,
        19: false,
        25: false
    },
    GainExperience: {
        1: false,
        10: false,
        13: false,
        17: false,
        19: false,
        25: false
    }
}

function initWebsocket(environment) {
    let path = 'wss://push.planetside2.com/streaming?environment=' + environment + '&service-id=s:' + config.censusServiceId;
    consoleLogger.status('censusWebsocketStatus', 'Initializing websocket connection to Census (' + environment + ')');
    consoleLogger.status('censusWebsocketStatus', path);

    websocketClient = new ws(path);

    // Register event handlers
    websocketClient.on('message', function (message) {
        routerPush.onrawmessage(message);
    });

    websocketClient.on('open', function () {
        consoleLogger.status('censusWebsocketStatus', 'Websocket Open!');
        fireMetagameSubscriptions();
    });

    websocketClient.on('error', function (error) {
        consoleLogger.error('censusWebsocketStatus', 'WEBSOCKET ERROR: ' + error);

        websocketClient = null; // Destroy the websocket instance
        setTimeout(function() {
            consoleLogger.error('censusWebsocketStatus', 'Reconnecting websocket...');
            initWebsocket(environment);
        }, 5000);
    });
};

function sendMessage(message) {
    consoleLogger.debug('censusWebsocketMessages', 'Sending to Census: %j', message);
    websocketClient.send(JSON.stringify(message));
}

function fireMetagameSubscriptions() {
    for (var key in config.supplementalConfig.pcWorlds) {
        let world = String(config.supplementalConfig.pcWorlds[key]);
        let message = {
        	"service":"event",
        	"action":"subscribe",
        	"eventNames":["MetagameEvent"],
            "worlds" : [world] // Has to be like this for census to pick it up
        };

        consoleLogger.debug('censusWebsocketMessages', 'Firing MetagameEvent Subscription - World: ' + world);
        sendMessage(message);
    }
}

module.exports = {
    'init': () => {
        initWebsocket('ps2');
    },
    'sendMessage': (message) => {
        sendMessage(message);
    },
    'pushChannels': [],
    'intChannels': [],
};
