...
function initWebsocket(environment) {
    ...
    consoleLogger.info('censusEvents', 'Initializing websocket connection to Census (' + environment + ')');
    consoleLogger.info('censusEvents', path);
    ...
    websocketClient.on('error', function (error) {
        consoleLogger.error('censusEvents', error);

        websocketClient = null; // Destroy the websocket instance
        setTimeout(function() {
            consoleLogger.info('censusEvents', 'Reconnecting websocket...');
