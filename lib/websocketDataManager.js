(function () {
    ...
    obj.subscribeToActives = function() {
        _.forEach(activeAlerts, function(value, key) {
            obj.subscribeStreams(value.world, value.zone);
        });
    }

    obj.subscribeStreams = function(world, zone) {
        if (! subscriptions[world]) {
            ...
            consoleLogger.error('websocketDataManager:subscribeStreams', 'Detected active subscription for world '+world);
        }
    }

    obj.unsubscribeStreams = function(world, zone) {
        if (subscriptions[world]) {
            // Send event to subscriber module
            ...
            consoleLogger.error('websocketDataManager:sendSubscription', 'Couldn\'t find subscription for world '+world);
        }
    }
    ...
    observer.subscribe(this, 'endedAlert', function(who, alert) {
        ...
        obj.endAlert(alert);
        obj.unsubscribeStreams(alert.world, alert.zone);
    })
